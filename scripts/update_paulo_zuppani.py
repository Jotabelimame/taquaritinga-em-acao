...
FOTOS_PAULO = "https://photos.app.goo.gl/6gRJ7JWaZhxB2fvk8"

for loc in locais:
    if loc["id"] == 15 or "paulo zupanni" in loc["nome"].lower() or "paulo zuppani" in loc["nome"].lower():
        loc["nome"] = "Avenida Paulo Zuppani"
        loc["linkFotos"] = FOTOS_PAULO
        loc["fotos"] = FOTOS_PAULO
        loc["status"] = "concluido"
        print(f"Atualizado Local #{loc['id']}: {loc['nome']} -> Status: CONCLUIDO | Fotos: {FOTOS_PAULO}")
