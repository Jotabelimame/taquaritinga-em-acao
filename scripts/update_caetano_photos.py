...
CAETANO_FOTOS = "https://photos.app.goo.gl/bvSxyHa8EtgdY9x16"

for loc in locais:
    if "caetano decaro" in loc["nome"].lower():
        loc["linkFotos"] = CAETANO_FOTOS
        loc["fotos"] = CAETANO_FOTOS
        loc["status"] = "concluido"
        print(f"Atualizado Local #{loc['id']}: {loc['nome']} -> Status: CONCLUIDO | Fotos: {CAETANO_FOTOS}")
...
print("\n=== FOTOS DA AVENIDA CAETANO DECARO SALVAS ===")
