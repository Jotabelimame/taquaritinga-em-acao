...
MODESTO_FOTOS = "https://photos.app.goo.gl/7k2SoaRFMmrc1wM56"

for loc in locais:
    if loc["id"] == 21 or "modesto" in loc["nome"].lower():
        loc["nome"] = "EMEB Prof. Modesto Bohrer / Modesto de Souza Dias"
        loc["linkFotos"] = MODESTO_FOTOS
        loc["fotos"] = MODESTO_FOTOS
        loc["status"] = "concluido"
        print(f"Atualizado Local #{loc['id']}: {loc['nome']} -> Status: CONCLUIDO | Fotos: {MODESTO_FOTOS}")
...
print("\n=== FOTOS DA EMEB PROF. MODESTO BOHRER SALVAS ===")
