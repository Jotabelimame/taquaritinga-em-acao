...
CALIL_FOTOS = "https://photos.app.goo.gl/JeSnuDeX5RJveKpx8"

for loc in locais:
    if "calil" in loc["nome"].lower():
        loc["linkFotos"] = CALIL_FOTOS
        loc["fotos"] = CALIL_FOTOS
        loc["status"] = "concluido"
        print(f"Atualizado Local #{loc['id']}: {loc['nome']} -> Status: CONCLUIDO | Fotos: {CALIL_FOTOS}")
...
print("\n=== FOTOS DA AVENIDA CALIL JOSE DIB SALVAS ===")
